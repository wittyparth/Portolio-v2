...
export function meta({ }: Route.MetaArgs) {
    return [
        { title: "Partha Saradhi - Personal Runtime" },
        { name: "description", content: "A live look at what I'm reading, watching, and listening to." },
    ];
}

export default function PersonalPage({ embedded = false }: { embedded?: boolean }) {
    return (
        <PageLayout embedded={embedded} className={embedded ? "bg-transparent" : "bg-[#111318]"}>
            <div className="text-white font-display relative overflow-x-hidden">
                {/* Background effects - only render when not embedded */}
                {!embedded && (
                    <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-0">
                        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#2b6cee]/5 rounded-full blur-[120px]" />
                        <div className="absolute bottom-[-10%] right-[-10%] w-[30%] h-[30%] bg-blue-500/5 rounded-full blur-[100px]" />
                    </div>
                )}

                {!embedded && <GlobalHeader />}
                {!embedded && <HeaderSpacer />}
                ...
                <main className="relative z-10 w-full max-w-6xl mx-auto flex flex-col gap-6 px-4 lg:px-8 py-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-auto lg:h-[620px]">
                        ...
                        <div className="lg:col-span-5 glass-panel rounded-2xl p-6 md:p-8 flex flex-col relative overflow-hidden group hover:border-[#2b6cee]/20 transition-all duration-300 border border-white/5">
                            ...
                            <button className="mt-6 w-full py-3 rounded-lg border border-white/10 bg-[#15171e]/50 hover:bg-[#2b6cee]/10 hover:border-[#2b6cee]/30 transition-all flex items-center justify-center gap-2 text-sm font-medium text-gray-300 hover:text-white z-10 group/btn">
                        ...
                        <div className="lg:col-span-7 flex flex-col gap-5">
                            {/* Anime Card */}
                            <div className="flex-1 glass-panel rounded-2xl p-6 relative overflow-hidden group cursor-pointer border border-white/5 hover:border-[#2b6cee]/40 transition-all duration-300">
